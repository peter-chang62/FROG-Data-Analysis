...
data = np.genfromtxt("12-22-2023/HNLF_input_corrected_pol.txt")
t_fs = data[:, 0][1:]
wl_nm = data[0][1:]
data = data[:, 1:][1:]
...
pulse_autoconv = light.Pulse.Sech(
    n_points=256,
    v_min=coll_autoconv.v_grid.min(),
    v_max=coll_autoconv.v_grid.max(),
    v0=(coll_autoconv.v_grid.max() - coll_autoconv.v_grid.min()) / 2
    + coll_autoconv.v_grid.min(),
    e_p=1e-9,
    t_fwhm=200e-15,
    time_window=coll_autoconv.t_grid.max() - coll_autoconv.t_grid.min(),
)
...
b, a = scipy.signal.butter(N=4, Wn=0.03, btype="low")
data_filt = scipy.signal.filtfilt(b, a, data, axis=1)

# symmetrize once the fringes have been filtered out.
data_filt = (data_filt.T + data_filt.T[::-1]).T / 2

# %% ----- background subtraction
shg = np.genfromtxt("12-22-2023/HNLF_input_corrected_pol_shg_bckgnd.txt")
t_fs = shg[:, 0][1:]
wl_nm = shg[0][1:]
shg = shg[:, 1:][1:]
shg[:, : idx[0]] = 0
...
pulse = light.Pulse.Sech(
    ...
    e_p=1e-9,
    t_fwhm=200e-15,
    time_window=(coll_retrvl.t_grid.max() - coll_retrvl.t_grid.min()) * 7,
)
